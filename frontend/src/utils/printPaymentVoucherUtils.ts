...
export interface PrintPaymentVoucherOptions {
...
export interface PrintPaymentVoucherResult {
  success: boolean;
  message: string;
  error?: string;
}
...
/**
 * Generate payment voucher content HTML
 */
const generatePaymentVoucherContent = (
  receipt: CustomerReceipt,
  customer: Customer | null,
  settings: Record<string, unknown>,
  printerType: 'thermal' | 'a4' | 'thermal-80mm' | 'thermal-58mm'
  ...
  return `
    <div class="payment-voucher-content font-arabic" style="text-align: center; direction: rtl; font-family: '${(settings?.bill_font_body as string) || 'Cairo'}', Arial, sans-serif; color: #000000; max-width: 800px; margin: 0 auto; padding: 20px;">
      
      <!-- Header Section -->
      ${((settings?.bill_show_logo as boolean) || (settings?.bill_show_company_info as boolean)) ? `
        <div class="payment-voucher-header" style="margin-bottom: 30px; border-bottom: 3px solid #000; padding-bottom: 20px;">
      ...
      <div class="payment-voucher-info" style="margin-bottom: 30px;">
      ...
      <!-- Footer Section -->
      <div class="payment-voucher-footer-section" style="margin-top: 30px;">
        <!-- Footer -->
...
/**
 * Print payment voucher with preview
 */
export const printPaymentVoucherWithPreview = async (
  receipt: CustomerReceipt,
  customer: Customer | null,
  settings: Record<string, unknown>,
  ...
  try {
    const voucherContent = generatePaymentVoucherContent(receipt, customer, settings, printerType);
    ...
    printWindow.document.write(`
      ...
      <html dir="rtl" lang="ar">
        <head>
          ...
          <title>معاينة طباعة سند صرف - ${receipt.receipt_number}</title>
          <style>
            @page {
        ...
        <body>
          <button class="print-button" onclick="window.print()">طباعة سند صرف</button>
          ${voucherContent}
        </body>
      </html>
    `);
...
/**
 * Quick print payment voucher
 */
export const quickPrintPaymentVoucher = async (
  receipt: CustomerReceipt,
  customer: Customer | null,
  settings: Record<string, unknown>,
  printerType: 'thermal' | 'a4' | 'thermal-80mm' | 'thermal-58mm' = 'a4'
  ...
  try {
    ...
    console.error('Error quick printing payment voucher:', error);
    throw error;
  }
};
