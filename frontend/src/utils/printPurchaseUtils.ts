...
export interface PrintPurchaseOptions {
  purchase: Purchase;
...
/**
 * Generate purchase bill content HTML
 */
const generatePurchaseContent = (
  purchase: any,
  supplier: Supplier | null,
  settings: any,
  printerType: 'thermal' | 'a4' | 'thermal-80mm' | 'thermal-58mm'
  ...
  return `
    <div class="purchase-content font-arabic" style="text-align: center; direction: rtl; font-family: '${settings?.bill_font_body || 'Cairo'}', Arial, sans-serif; color: #000000;">
      
      <!-- Header Section -->
      ${(settings?.bill_show_logo || settings?.bill_show_company_info) ? `
        <div class="purchase-header" style="margin-bottom: 20px; border-bottom: 2px solid #000; padding-bottom: 10px;">
      ...
      <div class="purchase-info" style="margin-bottom: 24px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          <span style="font-weight: bold; color: #000000;">رقم المشترى:</span>
          <span style="color: #000000;">${purchase.invoice_no || 'غير محدد'}</span>
        </div>
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          <span style="font-weight: bold; color: #000000;">التاريخ:</span>
          <span style="color: #000000;">${formatDate(purchase.invoice_date)}</span>
        </div>
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          <span style="font-weight: bold; color: #000000;">الوقت:</span>
          <span style="color: #000000;">${formatTime(purchase.invoice_date)}</span>
        </div>
        ${supplier ? `
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          <span style="font-weight: bold; color: #000000;">اسم المورد:</span>
        ...
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          ...
          <span style="color: #000000;">${purchase.created_by_name}</span>
        </div>
        ` : ''}
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          <span style="font-weight: bold; color: #000000;">المبلغ المدفوع:</span>
          <span style="color: #000000;">${formatCurrency(purchase.paid_amount)}</span>
        </div>
        ${purchase.remaining_amount && purchase.remaining_amount > 0 ? `
        <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 8px; border-radius: 4px;">
          <span style="font-weight: bold; color: #000000;">المبلغ المتبقي:</span>
          <span style="color: #000000;">${formatCurrency(purchase.remaining_amount)}</span>
        </div>
        ` : ''}
      </div>

      <!-- Items Table -->
      <div class="purchase-table-container" style="margin-bottom: 20px;">
        <table class="purchase-table" style="width: 100%; border-collapse: collapse; border: 2px solid #000; font-size: ${isThermal ? '14px' : '12px'};">
          <thead>
            <tr style="background-color: #e5e7eb;">
              <th style="border: 1px solid #000; padding: 8px; font-weight: bold; color: #000000; text-align: right;">المنتج</th>
              <th style="border: 1px solid #000; padding: 8px; font-weight: bold; color: #000000; text-align: center;">الكمية</th>
              <th style="border: 1px solid #000; padding: 8px; font-weight: bold; color: #000000; text-align: center;">سعر الشراء</th>
              <th style="border: 1px solid #000; padding: 8px; font-weight: bold; color: #000000; text-align: center;">المجموع</th>
            </tr>
          </thead>
          <tbody>
            ${purchase.items && purchase.items.length > 0 ? purchase.items.map((item: any, index: number) => `
            ...
            <tr style="background-color: #f3f4f6; border-top: 2px solid #000;">
              ...
              <td style="border: 1px solid #000; padding: 8px; color: #000000; text-align: center; font-weight: bold;">
                ${formatCurrency(purchase?.total_amount || purchase?.subtotal || 0)}
              </td>
            </tr>
            <tr>
              <td colspan="4" style="border: 1px solid #000; padding: 8px; color: #000000; text-align: center; background-color: #f9fafb;">
                <div style="font-size: 14px; font-weight: bold;">
                  ${convertNumberToWords(purchase?.total_amount || purchase?.subtotal || 0)} دينار عراقي
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <!-- Footer Section -->
      <div class="purchase-footer-section" style="margin-top: 20px;">
        <!-- Footer -->
...
/**
 * Print purchase bill with preview
 */
export const printPurchaseWithPreview = async (
  purchase: Purchase,
  supplier: Supplier | null,
  settings: any,
  ...
  try {
    const purchaseContent = generatePurchaseContent(purchase, supplier, settings, printerType);
    ...
    printWindow.document.write(`
      ...
      <html dir="rtl" lang="ar">
        <head>
          ...
          <title>طباعة المشترى - ${purchase.invoice_no}</title>
          <style>
            @page {
        ...
        <body>
          <button class="print-button" onclick="window.print()">طباعة المشترى</button>
          ${purchaseContent}
        </body>
      </html>
    `);
...
/**
 * Quick print purchase bill
 */
export const quickPrintPurchase = async (
  purchase: Purchase,
  supplier: Supplier | null,
  settings: any,
  printerType: 'thermal' | 'a4' | 'thermal-80mm' | 'thermal-58mm' = 'a4'
  ...
  try {
    ...
    console.error('Error quick printing purchase:', error);
    throw error;
  }
};
