import type { SupplierPaymentReceipt } from '@/features/supplierPaymentReceipts/supplierPaymentReceiptsService';
import type { Supplier } from '@/features/suppliers/suppliersService';
import { convertNumberToWords } from '@/utils/convertNumberToWords';

export interface PrintSupplierPaymentVoucherOptions {
...
export interface PrintSupplierPaymentVoucherResult {
  success: boolean;
  message: string;
  error?: string;
}
...
/**
 * Generate supplier payment voucher content HTML
 */
const generateSupplierPaymentVoucherContent = (
  receipt: SupplierPaymentReceipt,
  supplier: Supplier | null,
  settings: Record<string, unknown>,
  printerType: 'thermal' | 'a4' | 'thermal-80mm' | 'thermal-58mm'
  ...
  return `
    <div class="supplier-payment-voucher-content font-arabic" style="text-align: center; direction: rtl; font-family: '${(settings?.bill_font_body as string) || 'Cairo'}', Arial, sans-serif; color: #000000; max-width: 800px; margin: 0 auto; padding: 20px;">
      
      <!-- Header Section -->
      ${((settings?.bill_show_logo as boolean) || (settings?.bill_show_company_info as boolean)) ? `
        <div class="supplier-payment-voucher-header" style="margin-bottom: 30px; border-bottom: 3px solid #000; padding-bottom: 20px;">
      ...
      <!-- Payment Voucher Information Grid -->
      <div class="supplier-payment-voucher-info" style="margin-bottom: 30px;">
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
          <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 12px; border-radius: 6px; background-color: #ffffff;">
            <span style="font-weight: bold; color: #000000; font-size: 14px;">رقم السند:</span>
          ...
          ${supplier ? `
          <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 12px; border-radius: 6px; background-color: #ffffff;">
            <span style="font-weight: bold; color: #000000; font-size: 14px;">المورد:</span>
            <span style="color: #000000; font-weight: 600; font-size: 14px;">${supplier.name}</span>
          </div>
          ` : ''}
          <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 12px; border-radius: 6px; background-color: #ffffff;">
      ...
      ${(receipt.reference_number || receipt.purchase_invoice_no || receipt.notes) ? `
      <div class="additional-details" style="margin-bottom: 30px;">
        ...
        <div class="additional-details-info" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
          ...
          <div style="display: flex; justify-content: space-between; border: 2px solid #000; padding: 12px; border-radius: 6px; background-color: #ffffff;">
            <span style="font-weight: bold; color: #000000; font-size: 14px;">فاتورة الشراء:</span>
            <span style="color: #000000; font-weight: 600; font-size: 14px;">${receipt.purchase_invoice_no}</span>
          </div>
          ` : ''}
      ...
      <!-- Footer Section -->
      <div class="supplier-payment-voucher-footer-section" style="margin-top: 30px;">
        <!-- Footer -->
...
/**
 * Print supplier payment voucher with preview
 */
export const printSupplierPaymentVoucherWithPreview = async (
  receipt: SupplierPaymentReceipt,
  supplier: Supplier | null,
  settings: Record<string, unknown>,
  ...
  try {
    const voucherContent = generateSupplierPaymentVoucherContent(receipt, supplier, settings, printerType);
    ...
    printWindow.document.write(`
      ...
      <html dir="rtl" lang="ar">
        <head>
          ...
          <title>معاينة طباعة سند صرف للموردين - ${receipt.receipt_number}</title>
          <style>
            @page {
        ...
        <body>
          <button class="print-button" onclick="window.print()">طباعة سند صرف للموردين</button>
          ${voucherContent}
        </body>
      </html>
    `);
    ...
    console.error('Error printing supplier payment voucher:', error);
    throw error;
  }
};

/**
 * Quick print supplier payment voucher
 */
export const quickPrintSupplierPaymentVoucher = async (
  receipt: SupplierPaymentReceipt,
  supplier: Supplier | null,
  settings: Record<string, unknown>,
  printerType: 'thermal' | 'a4' | 'thermal-80mm' | 'thermal-58mm' = 'a4'
  ...
  try {
    ...
    printWindow.document.write(`
      ...
      <html dir="rtl" lang="ar">
        <head>
          <meta charset="UTF-8">
          <title>طباعة سند صرف للموردين</title>
          <style>
            @page {
              size: ${printerType === 'thermal' ? '80mm auto' : 'A4'};
              margin: ${printerType === 'thermal' ? '2mm' : '10mm'};
    ...
    console.error('Error quick printing supplier payment voucher:', error);
    throw error;
  }
};
